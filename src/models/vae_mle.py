...
def build_encoder(input_dim, latent_dim, l2_penalty=1e-5):
    """Builds VAE encoder network.
    Hidden layers: Dense with elu activation.
    Output: z_mean, z_log_var, sampled z.
    """
...
# --- Decoder with softplus-variance ---
def build_decoder(output_dim, latent_dim, l2_penalty=1e-5):
    """Builds VAE decoder network.
    Hidden layers: Dense with elu activation.
    Output: mu (linear), logvar (softplus).
    """
...
# --- MLE-style VAE ---
class VAE(Model):
    """Maximum-likelihood VAE model.
    Encoder/decoder: Dense layers with elu activation.
    Options:
        beta: KL penalty weight
        kl_warmup: gradual KL ramp-up
    """
    ...
    def compute_loss_components(self, x, mu, logvar, z_mean, z_log_var):
            """Computes VAE loss components: reconstruction and KL divergence."""
            safe_logvar = ops.clip(logvar, -6.0, 2.0)
            sigma2 = ops.exp(safe_logvar)
    ...
    def test_step(self, data):
        ...
        "kl_loss": self.kl_tracker.result(),
        "z_std": self.z_std_tracker.result(),
        "val_z_std": self.z_std_tracker.result(),  # <- added
        "z_logvar_mean": self.z_logvar_mean_tracker.result(),
        "val_z_logvar_mean": self.z_logvar_mean_tracker.result()  # <- added
        }
...
class KLEpochTracker(Callback):
    """Callback to track current epoch for KL warmup in VAE."""
...
class VAETrainingDebugCallback(Callback):
    """Callback for VAE training debug output at epoch end."""
